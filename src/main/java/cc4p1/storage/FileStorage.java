...
public final class FileStorage implements Storage {
  ...
  private Object lockFor(Path f) {
    return FILE_LOCKS.computeIfAbsent(f, k -> new Object());
  }
  ...
  public Optional<Account> getCuenta(long id) {
    ...
    try (BufferedReader br = Files.newBufferedReader(file, StandardCharsets.UTF_8)) {
      ...
      for (String line; (line = br.readLine()) != null;) {
        String[] f = line.split(";");
        if (Long.parseLong(f[0]) == id)
          return Optional.of(Account.fromCsv(f));
      }
      ...
      synchronized (lockFor(file)) {
        ...
        if (lines.isEmpty()) {
          ...
          for (int i = 1; i < lines.size(); i++) {
            ...
            if (id != acc.id())
              out.add(ln); // filtra la cuenta que vamos a reemplazar
          }
        }
  ...
  public java.util.stream.Stream<cc4p1.model.Account> scanCuentasPartition(int p) {
    ...
    try {
      ...
      // lista.
      // Ventaja: no hay que cerrar nada fuera; el stream NO depende de un Reader
      // abierto.
      java.util.List<cc4p1.model.Account> accs = java.nio.file.Files
  ...
  public BigDecimal arqueoSaldosPartition(int p) {
  ...
  private Optional<cc4p1.model.Transaction> findTxInPartition(String txId, int p) {
    ...
    try (BufferedReader br = Files.newBufferedReader(f, StandardCharsets.UTF_8)) {
      ...
      for (String line; (line = br.readLine()) != null;) {
        ...
        if (cols.length < 5)
          continue;
        if (txId.equals(cols[0]))
          return Optional.of(cc4p1.model.Transaction.fromCsv(cols));
      }
      return Optional.empty();
    } catch (IOException e) {
      throw new UncheckedIOException(e);
  ...
  public void appendTransaccion(cc4p1.model.Transaction tx) {
    ...
    if (existing.isPresent()) {
      var e = existing.get();
      boolean sameCore = e.idCuenta() == tx.idCuenta() &&
  ...
  // Rutas
  private java.nio.file.Path prestamosFile(int p) {
    return base.resolve("partitions").resolve("prestamos_p" + p + ".csv");
  }

  private java.nio.file.Path pagosFile(int p) {
    return base.resolve("partitions").resolve("pagos_p" + p + ".csv");
  }
  ...
  // Append de pago (idempotencia opcional por payId: puedes replicar la lógica de
  // transacciones)
