...
public final class WorkerServer {
  ...
  public WorkerServer(String nodeId, String host, int port, Path nodeBase, int numParts) throws IOException {
    ...
    if (!Files.exists(txLogPath))
      Files.createFile(txLogPath);
    loadTxIndex();
    recoverFromTxLog();
  }
    ...
    public void handle(HttpExchange ex) throws IOException {
      ...
      try {
        ...
        String body = new StringBuilder(256)
            ...
            .append(",\"latMicros\":{\"p50\":").append(p50).append(",\"p95\":").append(p95).append(",\"p99\":")
            .append(p99).append("}")
            ...
            .append(",\"chaos\":{\"latencyMs\":").append(chaosLatencyMs).append(",\"diskFail\":").append(chaosDiskFail)
            .append("}")
            .append("}")
            .toString();
      ...
      try {
        ...
        try {
          ...
          if (accFrom == null || accTo == null) {
            // Coord espera NOT_FOUND
            cacheAndReply(ex, txId, 404, "{\"ok\":false,\"error\":\"NOT_FOUND\"}");
            txFail.incrementAndGet();
            return;
          }
          var saldoFrom = balanceOf(accFrom);
          if (saldoFrom.compareTo(monto) < 0) {
            // Coord mapea SALDO_INSUFICIENTE → 409
            sendJson(ex, 409, "{\"ok\":false,\"error\":\"SALDO_INSUFICIENTE\"}");
            txFail.incrementAndGet();
            return;
          }
  ...
  final class CrearPrestamoHandler implements HttpHandler {
    ...
    public void handle(HttpExchange ex) throws IOException {
      ...
      try {
        ...
        // Aceptar ambos esquemas: por cuentaId (deriva idCliente) o por idCliente
        // directo (desde Coordinator)
        long cuentaId = parseLongOr(m.get("cuentaId"), parseLongOr(q.get("cuentaId"), -1));
        long idClienteArg = parseLongOr(m.get("idCliente"), parseLongOr(q.get("idCliente"), -1));
        String montoStr = m.containsKey("monto") ? m.get("monto") : q.get("monto");
        // tasa puede venir como "tasa" o "tasaAnual"
        String tasaStr = m.containsKey("tasa") ? m.get("tasa")
            : (m.containsKey("tasaAnual") ? m.get("tasaAnual")
                : (q.get("tasa") != null ? q.get("tasa") : q.get("tasaAnual")));
        String loanIdStr = m.containsKey("loanId") ? m.get("loanId") : q.get("loanId"); // opcional (idempotencia)

        if ((cuentaId <= 0 && idClienteArg <= 0) || montoStr == null) {
          sendJson(ex, 400, "{\"ok\":false,\"error\":\"VALIDACION\",\"msg\":\"Faltan idCliente/cuentaId o monto\"}");
          txFail.incrementAndGet();
          return;
        }
        BigDecimal monto;
        ...
        // Determinar idCliente: preferir parámetro directo; si no, derivar desde cuenta
        long idCliente;
        if (idClienteArg > 0) {
          idCliente = idClienteArg;
        } else {
          var accOpt = storage.getCuenta(cuentaId);
          if (accOpt.isEmpty()) {
            sendJson(ex, 404, "{\"ok\":false,\"error\":\"NOT_FOUND\",\"msg\":\"Cuenta inexistente\"}");
          ...
          idCliente = clientIdOf(accOpt.get());
        }
        ...
        // Idempotencia por loanId SIN getPrestamoById: busca el mismo id en préstamos
        // del cliente
        try {
          var existing = storage.getPrestamosByCliente(idCliente)
              .filter(l -> {
                ...
                if (v == null)
                  return false;
                try {
                  return Long.parseLong(String.valueOf(v)) == loanId;
                } catch (Exception e2) {
                  return false;
                }
              })
          ...
          if (existing.isPresent()) {
            ...
            String resp = new StringBuilder(160)
                ...
                .append(",\"pendiente\":")
                .append(pendiente == null ? "\"" + jsonEscape(monto.toString()) + "\"" : jsonValue(pendiente))
                .append("}")
                .toString();
        ...
        try {
          try {
            ...
            try {
              var ctor = Loan.class.getDeclaredConstructor(long.class, long.class, BigDecimal.class, BigDecimal.class,
                  LocalDate.class);
  ...
  private long before(HttpExchange ex) {
    ...
    if (!inFlight.tryAcquire()) {
      try {
        sendJson(ex, 503, "{\"ok\":false,\"error\":\"BUSY\"}");
      } catch (IOException ignore) {
      }
  ...
  private void after(long t0) {
    ...
    if (latSamplesMicros.size() < 5000)
      latSamplesMicros.add(micros);
    inFlight.release();
  }
  ...
  private void cacheAndReply(HttpExchange ex, String txId, int code, String body) throws IOException {
    if (txId != null && code == 200)
      txCache.putIfAbsent(txId, body);
