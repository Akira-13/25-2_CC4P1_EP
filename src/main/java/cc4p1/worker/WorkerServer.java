  ...
  final class CrearPrestamoHandler implements HttpHandler {
    ...
    public void handle(HttpExchange ex) throws IOException {
      ...
      try {
        ...
        // Si no viene cuentaId, usar idCliente (coordinador y CLI lo mandan así)
        if (cuentaId <= 0) {
          cuentaId = parseLongOr(m.get("idCliente"), parseLongOr(q.get("idCliente"), -1));
        }
        String montoStr = m.containsKey("monto") ? m.get("monto") : q.get("monto");
        String tasaStr = m.containsKey("tasa") ? m.get("tasa") : q.get("tasa");
        // Si no viene "tasa", usar "tasaAnual" (coordinador lo manda así)
        if (tasaStr == null) {
          tasaStr = m.containsKey("tasaAnual") ? m.get("tasaAnual") : q.get("tasaAnual");
        }
